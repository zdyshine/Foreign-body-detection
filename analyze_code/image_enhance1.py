...
max_pic_num = 10  # define one class of dog total picture numbers.
...
def GetDataPath():
    ...
    flags = tf.app.flags
    flags.DEFINE_string("data_path", "../image/", "directory of dogs, for enhance data.")
    flags.DEFINE_string("enhance_data_path", "./image/enhance/", "directory for store enhance data.")
...
def RandEnhancePicture(picname, savepath):
    ...
    image_decode_jpeg = tf.image.convert_image_dtype(image_decode_jpeg,
                                                     dtype=tf.float32)  # convert image dtype to float
    ...
    if rand == 1:  # random adjust brightness 在某范围随机调整图片亮度
        image_random_brightness = tf.image.random_brightness(image_decode_jpeg, max_delta=0.2)
    ...
    if rand == 2:  # random adjust contrast 在某范围随机调整图片对比度
        image_random_contrast = tf.image.random_contrast(image_decode_jpeg, 0.1, 0.3)  # 0.5,3修改0.5,2（改-0.5到2试试）
    ...
    if rand == 3:  # random adjust hue 在[-max_delta, max_delta]的范围随机调整图片的色相。max_delta的取值在[0, 0.5]之间
        image_random_hue = tf.image.random_hue(image_decode_jpeg, max_delta=0.2)
    ...
    if rand == 4:  # random adjust saturation 在某范围随机调整图片饱和度
        image_random_saturation = tf.image.random_saturation(image_decode_jpeg, 0.1, 0.3)
    ...
    if rand == 5:  # adjust gamma
        image_adjust_gamma = tf.image.adjust_gamma(image_decode_jpeg, gamma=0.1)
    ...
    if rand == 6:  # 自动插值
        image_resize_images = tf.image.resize_images(image_decode_jpeg, [700, 700], method=3)
    ...
    if rand == 7:  # 自动中央截取
        image_resize_image_with_crop_or_pad = tf.image.resize_image_with_crop_or_pad(image_decode_jpeg, 700, 700)
        image_resize_image_with_crop_or_pad = tf.image.convert_image_dtype(image_resize_image_with_crop_or_pad,
                                                                           dtype=tf.uint8)
    ...
    with tf.Session() as sess:  # create tensorflow session
        ...
        img_brightness, img_contrast, img_hue, img_saturation, img_gamma, img_8, img_9 \
            = sess.run([image_random_brightness, image_random_contrast,
                        image_random_hue, image_random_saturation, image_adjust_gamma, image_resize_images,
                        image_resize_image_with_crop_or_pad])
...
def EnhanceData():
    ...
    data_path = '/media/jinhui/1547059EF1791019/户外异物检测/Desktop/code/image/'
    enhance_path = '/media/jinhui/1547059EF1791019/户外异物检测/Desktop/code/enhance/'
